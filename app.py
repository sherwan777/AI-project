...
def Blur_Faces():
    ...
    try:
        ...
        while i < 1000:
            ...
            if not ret:
                break
            if i % 5 == 0:
                blurred_frame, windows = blur_matching_faces(frame, net, picture_encodings)
                face_count = len(windows)
                detected_faces_per_frame.append(face_count)
    ...
    # plt.title("Acuuracy of the model")
    # plt.xlabel("Frame number")
    # plt.ylabel("Number of faces detected")
    # plt.show()
        
        return {"video_output_path": "VIDEO PROCESSED"}, 200 
...
def get_video(filename):
    ...
    return send_file(filename,headers=headers)    
